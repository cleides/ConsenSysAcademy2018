...
contract BasicInfoGetter {
	...
	function getCurrentMinerAddress()
	public
	view
	returns (address) // get CURRENT block miner's address, 
	...
	function getCurrentGaslimit()
	public
	view
	returns (uint)  // the most gas that can be spent on any given transaction right now
	{													  
    ...
    public 
    view 
    returns (uint) // returns current block timestamp in SECONDS (not ms) from epoch
    {													
    ...
    public
    pure
    returns (bytes) 		// The data of a call to this function. Always returns "0xc8e7ca2e" for me.
    ...
    public
    view 
    returns (address)  // Returns the address of whomever made this call
    ...
    public
    payable
    returns (uint)		// returns amt of wei sent with this call
    {
    	return msg.value;
    }
    ...
    returns (uint)        
    {													
    	return gasleft(); //called a blobal function called "gasleft"
    }
    
	function getTxGasprice() public 
	view
	returns (uint) 	// "gasprice" is the amount of gas the sender was *willing* to pay. 50000000 for me. (geth default)
    {											     	
    	return tx.gasprice;
    }
    ...
    view
    returns (address) 	// returns sender of the transaction
